...
public class MachinePlayer extends Player {
  ...
  public Move chooseMove() {
		Move holder = new Move(); //***POSSIBLE BUG: we might be returning a quitmove if the invalidnodeexception occurs
		try {
			holder = minimax(chipcolor, -1, 1).move;
   ...
   */
  public Best minimax(int side, int alpha, int beta) throws InvalidNodeException { //side will tell us whose minimax its calling
	  Best myBest = new Best(); //will be updated to mybest possible move
	  Best reply; //will be updated to opponents best possible move
	  int x = gameboard.isNetwork(chipcolor); //returns whether we won, lost, or need to keep playing
	  
	  if (x == 1 || x == -1) { //base case: if you won or lost
		  myBest.score = x; 
		  return myBest;  
	  }
	  ...
	  DList legalmoves = allLegalMoves(side); //look at all the legalmoves of one side
	  ListNode first = legalmoves.front(); 	
	  try { //***POSSIBLE BUG: don't know what happens when there's a try and a throw??
		  while (first.isValidNode()){ 
			  gameboard.doMove((Move)first.item(), side); //Modifies "this" Grid
			  reply = minimax((side + 1) % 2, alpha, beta);
			  gameboard.undoMove((Move)first.item(), side); //Restores "this" Grid
  ...
  //to make the score between 1 and -1 make sure you divide by the maximum number of combinations that there can possible be
	  ...
	  for (int x = 0; x < 8; x++) {
		  for (int y = 0; y < 8; y++) {
			  ...
			  if (gameboard.findPiece(x, y) == opponentcolor) {
				  counterOpponent += gameboard.singleConnections(x, y).length();
			  }
		  }
	  }
	  float score = (counter - counterOpponent)/80;
	  return score; 
  }
  ...
  public boolean moveOutOfBounds(Move m) {
	  if (m.x1<0 || m.x1>7 || m.y1<0 || m.y1>7 || m.x2<0 || m.x2>7 || m.y2<0 || m.y2>7) {
		  return true;
	  }
	  return false; 
  }
  ...
  public boolean islegal(Move m, int chipcolor) {
	  ...
	  //stepmoves that should be addmoves and stepmoves that try to move the other team
  ...
  /*
   * if add move
   * 	for loop through all empty array slots
   * 		if m is legal on that slot store it
   * if step move
   * 	for loop through ten times total (the number of total pieces)
   * 	the piece to move = a specific chosen piece
   * 		for loop through all the possible step moves of that chosen piece
   * 			store it
   */
  public DList allLegalMoves(int chipcolor) {
	  ...
	  if (gameboard.getPieces(chipcolor) == 0) {
		  ...
		  for (int i = 0; i < moveHolder.length; i++) {
			  Move current = moveHolder[i]; //***POSSIBLE BUG: holds addMoves only!!! not stepMoves... abstraction?
			  for (int x = 0; x < 8; x++) {
				  for (int y = 0; y < 8; y++) {
					  if (gameboard.findPiece(x, y) == (Board.EMPTY)) {
						  Move tempMove2 = new Move(x, y, current.x1, current.y2); //***POSSIBLE BUG: not sure if coordinates are right
