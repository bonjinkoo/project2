...
public class DList extends List {
	...
	public DList() {
		...
		head = newNode(null, null, this.head, this.head);
		head.next = head;
		head.prev = head;
		size = 0;
	}
	...
	public void insertFront(Object item) {
		...
		// the `list' field (second parameter) as well.
		if (isEmpty()) {
			head.next = newNode(item, this, head, head); // new node attaches;
															// head's

			// next attaches
			head.prev = head.next; // head's prev attaches
			size++; // increment size
		} else {
			head.next = newNode(item, this, head, head.next); // create front
																// node
			head.next.next.prev = head.next; // set the second nodes previous to
												// the front node
			size++;
		}
	}
